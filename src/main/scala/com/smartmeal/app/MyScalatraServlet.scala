...
object MyLPProblem {
   def solve(breakfast: Int, lunch: Int, dinner: Int) =  {
      ...
      (temp filter ( _._2 == 1.0)) map {case (k,v) => (primary(k).concat(second(k)), cal(k).toString, carb(k).toString, sugar(k).toString, fat(k).toString, prot(k).toString, calcium(k).toString)}
